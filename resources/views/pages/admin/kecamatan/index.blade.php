                        ...
                        <span class="d-sm-inline">
                            <div class='dropdown'>
                                ...
                                <div class='dropdown-menu dropdown-menu-end'>
                                    ...
                                    <a class='dropdown-item' href='#' data-bs-toggle="modal"
                                        data-bs-target="#importKecamatanModal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                {{ $dataTable->table(['class' => 'table table-vcenter text-left'], true) }}
            </div>
        </div>
    </div>
@endsection
...
@section('modal')
            ...
            <div class="modal-content">
                <form id="editForm" action="#" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Kabupaten/Kota</label>
                            <select name="kabupaten_id" id="kabupaten_id_edit" class="form-select" required>
                                <option value="" selected disabled>- select options -</option>
                                @foreach ($kabupaten as $item)
                                    <option value="{{ $item->id }}">{{ $item->name }} -
                                        {{ $item->provinsi->name }}</option>
    ...
    <!-- Import Modal -->
    <div class="modal modal-blur fade" id="importKecamatanModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <form id="formImport" action="{{ route('kecamatan.import') }}" method="POST"
    ...
    <script>
        $(document).ready(function() {
            $('#editModal').on('show.bs.modal', function(e) {
                ...
                var kabupaten_id = $(e.relatedTarget).data('kabupaten_id');

                document.getElementById("editForm").action = url;
                $('#name_edit').val(name);
                $('#code_edit').val(code);
                $('#kabupaten_id_edit').val(kabupaten_id);
            });
        });
    </script>
@endsection
